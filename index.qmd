...
Slides: [slides.html](Final Presentation.html){target="_blank"}
...
[@lam_graphcast_2023][@keisler_gnn_2022].
...
-   Feature Pruning: Features missing more than 10% of their values in
    over 6 our of 8 stations were removed.
...
simplifies the graph structure of the weather stations.
...
The GNN, like the linear model, uses the preceding 28 time steps to
forecast the next temperature value. However, it is equipped to model
both temporal trends and spatial interdependencies, which the linear
model is not explicitly designed to capture.
...
Initial exploratory analysis focused on filtering out low-quality data
and reducing the dimensionality of the dataset:
...
-   Edge weights wwere defined as the inverse of the geodesic distance,
...
continuous data, as visually confirmed during quality checks and shown
below.
...
all stations. This structure enabled the model to learn boht temporal
...
trends of the weather data with little latency in predictive results.
...
When both models are compared against eachother per station it becomes
...
-   A Graph Neural Network trained on a spatiotemporal weather graph may
...
-   Careful data preprocessing, including imputation's scaling, and
...
-   GNNs are incredibly sensitive to parameter tuning and may outperform
    if provided a much larger model structure.
